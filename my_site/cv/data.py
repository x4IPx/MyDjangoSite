# type here title you want
titleCV = "Django Personal CV"

# type here your name and surname
yourName = "Барков Николай"

# type here current profession
yourProfession = "Системный администратор Linux"

# type here your bio
yourBio = "Приветствую! Так как у меня небольшой опыт работы с Ansible, Docker, Git и PostgresSQL я составил резюме, которое демонстрирует мои навыки работы с этими инструментами."

# type here your current location
yourCountry = "Москва"

# if you want add another social media account, add here icon code of it
socialContact = [
    'fab fa-github',
#    'fab fa-linkedin',
#    'fab fa-instagram',
    'fab fa-vk',
    'fab fa-telegram',
    'fas fa-envelope',
    'fas fa-phone-alt'
]

# type here some contact
yourContact = [
    # github link
    'https://github.com/x4IPx/MyAnsible',
    # linkedin link
#    'https://linkedin.com',
    # instagram link
#   'https://instagram.com',
    # twitter link
    'https://vk.com/x4ipx',
    # mailto:...your email
    'https://t.me/Karambara',
    # facebook link
    'mailto:x4ipx91@gmail.com',
    # tel:...your phone no
    'tel:89652179849'
]

# type here your birthday and (your age)
yourBirthday = "09 Сентября 1991"

# type here your skills
yourSkills = [
    'GIT',
    'Docker',
    'Ansible',
    'SQL',
    'Администрирование ЛВС',
    'Администрирование Windiws/linux',
    'Python'
]

# type here your skills percent rate
skillsPercent = [
    'умею использовать Git и могу применять основные команды в своей работе',
    'работать с Docker-контейнерами, собирать их из YAML-файлов, настраивать и запускать.',
    'писать плейбуки, сайт полностью развернут с помощью ansible.',
    'работать с базами данных ,создавать востанавлевать бекапы, проводить регламентированные работы.',
    'настраивать сетевое оборудование, понимаю сетевой стек IPTables,VPN,DNS,DHCP.',
    'Настройка серверов, востановление работоспособности',
    'Junior. Я не программист, но умею использовать Python в работе.',
]

# type here your hobbies
yourHobbies = [
    'Бег',
    'Йога/медитация',
    'Танцы',
    'Катание на велосипеде'
]

# type here your certificates
yourCerts = [
    'University of Oxford Certificate',
    'Harvard University Certificate'
]

# type here your education info
yourEdu = [
    'Московский колледж управления и новых технологий',
    'Техническое обслуживание вычислительной техники и компьютерных сетей'
]

# type here your education years (start-end)
eduYear = [
    '2011',
    '2016'
]

# type here your work history
yourWork = [
    'Такском, ООО Техническая поддержка'
]

# type here your work years (start-end)
workYear = [
    'August 2016 - Present'
]

# type here your projects
yourProject = [
    'New Software Innovations'
]

# type here your projects link (if there is)
projectLink = [
    'http://domain.com'
]

# type here your extras (driving license etc.)
yourExtras = [
    'Coffee',
]

# footer text in here (please don't change)
footerText = "Django Personal CV developed by <a target='_blank' class='author' href='https://github.com/en0ndev'>en0ndev</a>"

# thank you for use this template.
